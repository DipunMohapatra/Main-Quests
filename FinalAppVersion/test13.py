...
def main():
    ...
    def analyze_sentiments_parallel(comments, batch_size=25):
        ...
        total_batches = max(1, len(valid_comments) // batch_size + (1 if len(valid_comments) % batch_size > 0 else 0))

        # Parallel processing of batches
        with ThreadPoolExecutor(max_workers=2) as executor:  # Adjust the number of workers if needed
            futures = []
            for i in range(0, len(valid_comments), batch_size):
                batch = valid_comments[i:i + batch_size]
                futures.append(executor.submit(sentiment_pipeline, batch))
            
            for i, future in enumerate(futures):
                results = future.result()
                sentiment_labels.extend([result['label'] for result in results])
                progress_bar.progress(min((i + batch_size) / len(valid_comments), 1.0))
                ...
                if pd.api.types.is_numeric_dtype(column_data):
                    ...
                    if st.button(f"Analyze Sentiments in {selected_column}"):
                        with st.spinner('Analyzing sentiments...'):
                            sentiment_labels, valid_comments = analyze_sentiments_parallel(comments)
