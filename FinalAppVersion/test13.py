...
if 'ngram_filter' not in st.session_state:
    st.session_state.ngram_filter = None
...
def main():
    ...
    def analyze_sentiments(comments, batch_size=50):
        ...
        total_batches = (total_comments + batch_size - 1) // batch_size  # Calculate total batches
        
        for i in range(0, total_comments, batch_size):
            batch = valid_comments[i:i + batch_size]
            results = sentiment_pipeline(batch)
            sentiment_labels.extend([result['label'] for result in results])
            # Ensure progress value does not exceed 1.0
            progress_bar.progress(min((i + len(batch)) / total_comments, 1.0))
    ...
    def plot_interactive_wordcloud(df, text_column, sentiment_filter):
        ...
        vectorizer = TfidfVectorizer(stop_words='english', max_features=50)  # Reduce max_features for faster processing
        try:
    ...
    # Generate wordcloud based on the selected sentiment
    ...
    # Function to plot histogram and descriptive statistics for numeric data
        ...
        if uploaded_sentiment_file:
            ...
            if selected_column:
                ...
                if pd.api.types.is_numeric_dtype(column_data):
                    ...
                    comments = column_data.dropna().tolist()
