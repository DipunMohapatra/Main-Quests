...
def load_time_series_data(uploaded_file, sheet_name: str) -> pd.DataFrame:
    ...
    data = pd.read_excel(uploaded_file, sheet_name=sheet_name, usecols=['Time', 'Value'])
    return data
...
def analyze_sentiments(df, text_column):
    ...
    df['sentiment_category'] = pd.cut(sentiment_scores, bins=[-1, -0.05, 0.05, 1], labels=['Negative', 'Neutral', 'Positive'])
    return df
    ...
    with analysis_tab:
        ...
        if uploaded_file:
            ...
            if sheet_name:
                ...
                model = auto_arima(train_residual['Residual'], seasonal=True, m=12, stepwise=True)
                residual_forecasts = model.predict(n_periods=len(test_residual))
                ...
                final_forecasts = trend_forecast + seasonal_forecast + residual_forecasts
                ...
                future_trend = np.tile(data['Trend'].iloc[-12:].values, (months_to_forecast // 12 + 1))[:months_to_forecast]
                future_seasonal = np.tile(data['Seasonal'].iloc[-12:].values, (months_to_forecast // 12 + 1))[:months_to_forecast]
                
                future_residual = model.predict(n_periods=months_to_forecast)
