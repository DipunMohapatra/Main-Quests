...
def analyze_sentiments(df, text_column):
    ...
    if transformer_results:
        df['sentiment_label'] = [result['label'] for result in transformer_results]
        df['sentiment_score'] = [result['score'] for result in transformer_results]
        st.write("Sentiment analysis completed.")
    else:
        st.error("No results from sentiment analysis.")
            ...
            if not df.empty:
                ...
                if selected_column:
                    if np.issubdtype(df[selected_column].dtype, np.number):
                        ...
                        sentiment_choice = st.radio("Select sentiment to view comments and word cloud:", ['Positive', 'Negative'])
