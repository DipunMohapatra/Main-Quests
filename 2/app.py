...
def analyze_sentiments(df, text_column, model_name):
    """
    ...
    """
    sentiment_pipeline = load_sentiment_pipeline(model_name)
    ...
    transformer_results = sentiment_pipeline(texts)
    ...
    for result in transformer_results:
        ...
        # Consider positive if score is greater than 0.6 and negative if less than 0.4
...
def main():
    ...
    with tabs[0]:
        ...
        selected_model = st.selectbox("Select a model for sentiment analysis:", list(model_options.keys()))
            ...
            if not df.empty:
                ...
                if selected_column:
                    if np.issubdtype(df[selected_column].dtype, np.number):
                        ...
                        # Analyze sentiments with the selected model
                        df = analyze_sentiments(df, selected_column, model_options[selected_model])
